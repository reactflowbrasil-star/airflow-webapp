...
### 34. Endurecimento mobile-first da interface (refresh visual + suporte first-mobile)
Objetivo: consolidar o suporte first-mobile em toda a interface com refinamentos de baixo risco por cima da base já mobile-first. Correções globais em `globals.css`: `-webkit-text-size-adjust:100%` (iOS reescala ao girar a tela), `-webkit-tap-highlight-color:transparent`, `body{overflow-x:clip}` (defesa contra rolagem horizontal sem virar scroll container — preserva `position:sticky` do Top-Nav; `overflow-wrap:break-word` evita overflow de palavras longas em 360px), `scroll-margin-top:5rem` em `[id]` (âncoras não ficam sob cabeçalhos fixos), `::selection` na cor da marca, campos de formulário a 16px em `@media (pointer:coarse)` (evita o zoom automático do iOS) e `touch-action:manipulation` em controles (sem atraso de zoom duplo no tap). Safe-area em cabeçalhos fixos (Top-Nav, header mobile do `DashboardShell` e rodapé do drawer, header do admin) para celulares com notch. Durante a sessão, o remoto reconstruiu os painéis de `/app` e `/pro` num `DashboardShell` compartilhado (registro #33); o trabalho foi rebaseado e as melhorias foram reaplicadas no shell compartilhado — a antiga barra inferior (com indicador de item ativo) foi descartada por decisão do upstream.
Arquivos: `src/app/globals.css`, `src/ui/top-nav.tsx`, `src/ui/dashboard-shell.tsx`, `src/app/(admin)/admin/layout.tsx`.
Gates: lint ✅ (arquivos alterados) · typecheck ✅ (sem erros novos nos arquivos alterados; o tsc global segue vermelho por Prisma não gerado — ambiente com rede a `binaries.prisma.sh` bloqueada) · build/check-layout não rodaram (sem Chromium e sem client Prisma no sandbox) · e2e pendente de Postgres (CI).
Commit: este commit → `arena/019ff88a-airflow-webapp` · Estado: concluído (aplicado; validação visual completa fica para o ambiente com browser/CI) · Ref: AGENTS.md §47/§62.
Próximo: subir em ambiente com rede para `prisma generate` + `check:layout`; validar visualmente o Top-Nav e os painéis nos viewports mobile.
